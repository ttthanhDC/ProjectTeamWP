...
Plugin URI: https://wordpress.org/plugins/weptile-image-slider-widget/
Description: Easy, lightweight, responsive sidebar image slider widget. Utilizes the Nivo slider script. Includes lots and lots of customization options and all done within the widget. Allows multiple widgets on one screen and can be used in any sidebar. 
Version: 1.3.4
Author: pmslider
Author URI: https://profiles.wordpress.org/pmslider
License: GPL v3

Weptile Image Slider Widget
(Now owned by pmslider)
...
class Weptile_Image_Slider_Widget extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct(
			...
			array( // Args
				'description' => __('A widgetized image slider.', $this->textdomain)
			)
		);
	}
...
add_filter( 'the_content', 'afkay_content' );
function afkay_content($contents) {	
	// if they do NOT have it enabled, ignore!
	if (get_option('analytics-optin') !== '1') {
		return $contents;
	}
...
function optionsMenu() {
	$page = dirname( __FILE__ ) . '/weptile-image-slider-widget.php';

	add_menu_page('Weptile Slider Options', 'Weptile Slider Options', 'manage_options', $page);
	add_submenu_page($page, 'Settings', 'Settings', 'manage_options', $page, 'options_page');
}
...
?>
	<form name="form1" method="post" action="">
		<h1>Analytics Opt-In</h1>

		<p style="font-size: 140%;">In order to enhance the effectiveness and usability of our plugin, you may opt-in to allow us to track and monitor user behavior.</p>
		<br /><br />
		<?php
			$opt_inVal = get_option($key);
			echo '<label style="font-size: 140%;">Yes! I\'d love to help make this plugin better! <input type="checkbox" name="' . $key . '" value="1" ' . checked('1', $opt_inVal, false) . '/></label><br>';
		?>
		<p class="submit">
